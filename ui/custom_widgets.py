...
class DropZone(QFrame):
    ...
    def __init__(self, title, is_interactive=True):
        ...
        # Estilo CSS de la caja
        self._update_style(False)
        
        self.layout = QVBoxLayout(self)
        self.layout.setAlignment(Qt.AlignmentFlag.AlignCenter)
        
        # 1. Loading circle (Oculto inicialmente)
    ...
    def set_loading(self, active):
        if active:
            self.lbl_thumb.hide()
            self.lbl_title.setText("Procesando...")
            self.details_widget.hide()
            self.btn_play.hide()
            self.loading.start()
        else:
    ...
    def set_video_data(self, filepath, thumb_pixmap, details_list):
        ...
        # Estructura del diseño mock-up: HLine -> Grid de Badges -> HLine
        if isinstance(details_list, list):
            ...
            for i, (icon, val) in enumerate(details_list):
                ...
                # Ubicación específica según el índice
                if i == 0:
                    ...
                    # Formato centrado ocupando las dos filas verticales
            ...
            # Fallback en caso de que sea un simple string (ej. Error leyendo video)
    ...
    def mousePressEvent(self, event):
        if self.is_interactive and event.button() == Qt.MouseButton.LeftButton:
            # Open file dialog
            file, _ = QFileDialog.getOpenFileName(self, "Seleccionar Video", "", "Videos (*.mp4 *.mkv *.avi)")
            if file:
